...
const profileSchema = z.object({
  ...
  dribbble: z.string().url('Enter a correct url').or(z.literal('')).optional(),
...
const EditProfilePage = ({ isOpen, onClose }: EditProfileProps) => {
  ...
  const form = useForm<z.infer<typeof profileSchema>>({
    ...
    defaultValues: {
      ...
      dribbble: user?.socialLinks?.find((link) => link.name === 'dribbble')?.link ?? '',
      ...
      anyother: user?.socialLinks?.find((link) => link.name === 'anyother')?.link ?? '',
    },
  });
  ...
  const onSubmit = async (values: z.infer<typeof profileSchema>) => {
    ...
    const userSocialLinksUpdationData = [
      ...
      { id: 'dribbble', name: 'dribbble', link: values.dribbble },
