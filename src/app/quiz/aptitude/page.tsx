...
const questionsData = [
    {
      id: '1',
      questionShortDesc: 'What is 2 + 2?',
      questionLongDesc: 'This is a simple arithmetic question. Please choose the correct answer.',
      aptitudeId: 'aptitude_1',
      options: [
        {
          id: '1a',
          optionText: '3',
    ...
    {
      id: '2',
      questionShortDesc: 'What is the capital of France?',
      questionLongDesc: 'This is a general knowledge question about European capitals.',
      aptitudeId: 'aptitude_2',
      options: [
        {
          id: '2a',
          optionText: 'Paris',
    ...
    {
      id: '3',
      questionShortDesc: 'Which planet is known as the Red Planet?',
      questionLongDesc: 'A science question about planets in our solar system.',
      aptitudeId: 'aptitude_3',
      options: [
        {
          id: '3a',
          optionText: 'Earth',
...
const AptitudeQuestionsPage = () => {
  ...
  return (
    <div className="bg-slate-50 h-screen w-screen px-4">
      <div className="lg:pl-20 pt-10 h-screen max-w-[1120px]">
        <h1 className="font-product-sans decoration-skip-none my-4 text-left text-[20px] font-bold leading-[24.26px] underline-offset-4">
          Aptitude Quiz
        </h1>
        <div className="my-[10px] flex max-h-[44px] max-w-[1120px] items-center justify-between">
          <span className={`text-[20px]leading-[24.26px] font-product-sans px-1 py-3 font-normal  ${timeLeft <=5*60 ? 'text-[#EA4335]' : 'text-[#0F9D58]'}`}>
            {formatTime(timeLeft)}
          </span>
          ...
          <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
              ...
              <DialogContent className="max-w-[425px] max-h-[310px] rounded-sm grid align-middle items-center">
                <div>
                  <DialogTitle className="text-red-500
                  ml-[40%] my-5">
                    <TriangleAlert className="w-[70px] h-[70px]" />
                  </DialogTitle>
                  <div>
                ...
                <div className="flex justify-between">
                  ...
                  <button onClick={() => setIsModalOpen(false)} className='bg-[#635BFF] text-white rounded-sm px-2 py-1'>No, Continue</button>
                </div>
              </DialogContent>
            </Dialog>
          </span>
        ...
        <div className="lg:flex grid  gap-6">
          <div className="max-w-[740px]">
            <div className="max-h-[155px] max-w-[740px] lg:h-[155px] lg:w-[740px] rounded-md bg-white pl-5 pt-6">
              <h3 className="font-product-sans text-4 mb-4 font-medium leading-[19.41px] text-[#100C2C]">
                Question 1
              </h3>
              <p className="font-product-sans text-4 font-normal leading-6 text-[3D3D3D]">
            ...
            {currentQuestion?.options.map((option) => ( 
              <li key={option.id} className="flex h-12 max-w-90 items-center gap-2 rounded-md bg-white px-5 py-[14px]">
                <input type="radio"  id={option.id} className="h-4 w-4 border-2" name={`question-${currentQuestion.id}`}
                    value={option.id}
            ...
            <div className="top-[475px] mt-20 flex max-w-[740px] justify-between">
              <button onClick={handlePrevious} className="h-[44px] w-[110px] rounded-sm bg-[#F1F1F1] text-[16px] font-medium leading-4 text-black" disabled={questions.findIndex((q) => q.id === currentQuestion?.id) === 0}
          >
                Previous
              </button>
              <div className="flex max-h-[44px] max-w-[286px] gap-6 ml-2">

                {currentQuestion?.id && (
                  <button className={`flex w-[112px] items-center gap-2 rounded-sm border bg-white  px-6 py-3 text-[#3D3D3D] `} onClick={() => handleMarkForReview(currentQuestion.id)}>
                    <Bookmark />
                ...
                <button className="h-[44px] w-[150px] rounded-sm bg-[#635BFF] py-3 text-[16px] font-medium leading-4 text-white"  onClick={handleNext} disabled={questions.findIndex((q) => q.id === currentQuestion?.id) === questions.length - 1}>
                  Save and Next
                </button>
              </div>
            </div>
