...
const QuizList : React.FC<QuizListProps> = ({ answeredCount, unansweredCount, markedCount, questionsCount }) => {
  ...
  return (
    <div className="h-[459px] max-w-[360px] rounded-sm bg-white ">
            <div className="ml-5 mt-6 grid h-[44px] w-[157px] gap-2">
              <h3 className="h-[19px] text-[16px] font-normal leading-[19.41px]">
                Design-Aptitude Test
              </h3>
              <p className="h-[17px] text-[14px] font-normal leading-4 text-[#6D6D6D]">
                Attempt all
              </p>
            </div>
            ...
            <div className="my-[28px] ml-2 h-[321px] w-[335px] ">
              ...
              <div className="grid h-[276px] w-[321px] grid-cols-5 gap-6">
                {questions.map((question, index) => {
                  ...
                  return (
                    <div key={question.id} className={`flex h-12 w-12 items-center justify-center border text-[14px] leading-4 tracking-[0.02em] text-[#100C2C] ${status === 'not-attempted' ? 'bg-gray-400 rounded-sm' : ''}
                    ${status === 'attempted' ? 'bg-[#56BA85] rounded-full' : ''}
                    ${status === 'reviewed' ? 'bg-[#F3B153] rounded-full' : ''}`} onClick={() => router.push(`/quiz/${aptitudeId}?questionId=${question.id}`)}>
                      {index + 1}
                    </div>
                  );
                })}
