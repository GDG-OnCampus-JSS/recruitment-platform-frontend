...
export function useDataTable<TData, TValue>({
  ...
  React.useEffect(() => {
    ...
    Array.from(searchParams.keys()).forEach((param: string) => {
      if (
        (searchableColumns.find((column) => column.value === param) &&
          !debouncedSearchableColumnFilters.find((column) => column.id === param)) ||
        (filterableColumns.find((column) => column.value === param) &&
          !filterableColumnFilters.find((column) => column.id === param))
      ) {
        Object.assign(newParamsObject, { [param]: null });
      }
    });

    // After cumulating all the changes, push new params
    enableQueryParams && router.push(`${pathname}?${createQueryString(newParamsObject)}`);
